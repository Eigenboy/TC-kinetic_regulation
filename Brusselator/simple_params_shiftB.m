...
k0=[1,2,1,1,10^(-5),10^(-6),10^(-6),10^(-6)];
E0=[15,25,20,20,20,20,20,20]*8.31*T(1);

M=98;
[P1,Dsp1,P2,Dsp2,Q10,K,DPDK,Z_min,Z_max]=deal(zeros(1,M));
RDP=zeros(8,M);
...
%         B(1,kk)=10^(2.05*kk-0.05)/2;
    ...
    if lambda1(1,1)>0
       ...
       %      [t3,z3]=ode23s(@(t,x) bruss_new(x,kv2,a),[0,30*pi/omega1],[1.1*x_st(1,1),1.1*y_st(1,1)]);
       %      [t3,z3]=ode23s(@(t,x) bruss_new(x,kv2,a),[0,60*pi/omega1],z3(end,:));
       %      P3=periods(t3,z3(:,1));
       %      RDP2(1,kk)=1/amp*(P3/P1(1,kk)-1);
       %      clear t3 z3;
       % else
       %      RDP(1,kk)=NaN;
       % end
