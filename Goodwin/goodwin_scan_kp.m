% Original Goodwin model; varying k_p
% Using central difference method for higher accuracy of period
% sensitivities
...
T=[298;308];
n=10;
Ed=[5,5,5]*8.31*T(1);Ek=[30,30,30]*8.31*T(1);
...
for jj = 1:M
    ...
    % k_p(1,jj) = 1.5*10^(0.02*(jj-1));
    k_p(1,jj) = 1.2 + 0.02*(jj-1);
    k(1,2) = k_p(1,jj);
    k(2,1:3)=k(1,1:3).*exp(Ek/(8.31*T(1))).*exp(-Ek/(8.31*T(2)));

    [~,X1]=ode45(@(t,x) Goodwin(x,k(1,:),d(1,:),n),[0,800],[1,0,0]);
    [t1,X1]=ode45(@(t,x) Goodwin(x,k(1,:),d(1,:),n),[0:0.02:1200],X1(end,:));
 % figure(1);plot(t1,X1);
 % figure(2);plot(X1(:,1),X1(:,3));hold on;
    ...
    if P1(1,jj)~=0
        ...
        for kk=1:6
            ...
            [~,X3]=ode45(@(t,x) Goodwin(x,kv(1,1:3),kv(1,4:6),n),[0,800],[1,0,0]);
            [t3,X3]=ode45(@(t,x) Goodwin(x,kv(1,1:3),kv(1,4:6),n),[0:0.02:1200],X3(end,:));
            P3=periods(t3(floor(length(t3)/2):end,1),X3(floor(length(t3)/2):end,3));
            [~,X4]=ode45(@(t,x) Goodwin(x,kv(2,1:3),kv(2,4:6),n),[0,800],[1,0,0]);
            [t4,X4]=ode45(@(t,x) Goodwin(x,kv(2,1:3),kv(2,4:6),n),[0:0.02:1200],X4(end,:));
            P4=periods(t4(floor(length(t4)/2):end,1),X4(floor(length(t4)/2):end,3));
            % figure(1);plot(t3,X3);
            % figure(2);plot(X3(:,1),X3(:,3));hold on;
            if P3 > 0 && P4 > 0
    ...
    [~,X2]=ode45(@(t,x) Goodwin(x,k(2,:),d(2,:),n),[0,800],[1,0,0]);
    [t2,X2]=ode45(@(t,x) Goodwin(x,k(2,:),d(2,:),n),[0:0.02:1200],X2(end,:));
